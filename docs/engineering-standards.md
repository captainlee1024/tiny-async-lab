...
当一页同时承担连续机制解释与固定源码/API 查阅两种可以独立验收的阅读任务时，先建立逐项内容去向表，再拆为相邻的机制页与参考页；机制页保留建立心智模型所需的因果链，参考页保留完整表面、stability 与证据边界，不能把移出主线变成知识删除。

组织正文前，必须先以固定版本为边界盘点与主问题直接相邻的表面，包括核心签名中的 receiver、关联类型、泛型和 lifetime，以及目标类型的 variants、methods、trait impls、macros、blanket impls、stability attributes 和 compiler-only hooks。
盘点中的每一项必须明确归入本章主推理链、推理完成后的参考材料、已有权威解释的链接或一个已命名的后续里程碑；不进入当前主线不等于可以无记录省略，源码中的排列顺序也不决定正文顺序。

核心签名或状态类型第一次承担推理时，必须逐层解释会改变机制结论的语法，包括 receiver 与值类别、所有权与借用、共享与可变、lifetime 参数与省略、wrapper 与 pointer 层次、关联类型、泛型约束、嵌套返回层次、unsafe 和 stability。
本地解释至少回答每一层约束谁、当前机制为什么需要它以及它不代表什么；如果完整证明属于独立机制，当前章节仍须给出足以支撑本章结论的操作含义，并指向具体 symbol、问题或里程碑，不能只写“后面再讲”。
只有省略后不会改变读者机制模型的普通语法才可以视为前置知识；承载状态、所有权、可变性、地址稳定性、lifetime、进度、失败、取消、安全或稳定性边界的细节，不能因为写法短小而跳过。

正文中的段落、表格和图应至少承担一项职责：建立或修正概念，说明因果或状态关系，提供证据，限定结论，给出必要例子，或者连接后续验证路径。
同一技术事实在最终读者正文中只保留一个主要表达载体：代码与教学注释负责局部语法、字段和短分支，表格负责精确映射与对比，图负责跨节点关系、状态和时序，正文负责原因、公共契约、例外与证据边界。
其他载体可以提供必要过渡或指向主要载体，但不能再次完整复述相同输入、分支或结论；改变表达载体后，必须对照检查代码注释、紧邻正文、表格、图以及章首章尾摘要，并删除已经被替代的旧表达。
...
- 节点名称与正文术语一致，箭头语义明确；
- 用图替代一组更难扫描的关系说明，不在保留等价文字后只附加重复摘要；正文仍说明最重要的不变量，并承担图难以表达的精确契约、例外和证据边界；
...
- 当解释能够逐项对应一个签名、字段或短状态转换时，优先使用经过编译测试的教学注释贴近对应语法，并删除被它等价替代的正文；mdBook 隐藏行只容纳 imports、assertions 等编译脚手架，可见片段仍须独立可读；依赖标准库内部上下文而无法独立编译的固定源码节选可以使用 `ignore`，但必须给出固定路径、说明省略内容和不能独立编译的原因；引用固定源码时还须明确标出改写和新增注释，代码无法直接证明的公共契约、设计原因与例外仍由正文和相应证据承担。
...
`make external-links` 只提供按需的链接可达性初筛；失败结果必须人工区分链接失效、限流、反自动化与临时网络故障，成功结果也不能证明来源仍然支持文档结论。
自动检查必须有明确的信噪比。
...
测试应归属于它验证的契约和状态所有者，但不要求所有测试代码都与实现写在同一个物理文件。

- 少量、只依赖当前模块私有细节的单元测试可以保留在实现文件底部的 `#[cfg(test)] mod tests` 中。
- 当测试逻辑已经影响核心实现的自上而下阅读、形成独立测试职责或需要持续增长时，将它移入单独测试模块；crate 根可以使用 `#[cfg(test)] mod tests;` 加 `src/tests.rs`，`src/foo.rs` 中的子模块可以使用同样声明加 `src/foo/tests.rs`。
- 只通过 public API 验证跨模块或 crate 级契约时，优先使用 package 根目录的 `tests/<scenario>.rs` integration test，不为测试暴露原本不需要公开的内部 API。
- 共享测试辅助代码只有在存在真实复用或需要集中维护测试不变量时才提取，避免建立与生产代码平行演化的测试框架。

拆出测试文件的理由是改善局部推理、所有权和 review 边界，不是单纯方便 Git staging。
实现与直接证明该实现所必需的测试仍然可以并且通常应该进入同一个原子 commit；独立 test commit 的条件由 [`CONTRIBUTING.md`](../CONTRIBUTING.md) 定义。
